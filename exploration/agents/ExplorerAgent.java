...
public abstract class ExplorerAgent implements sim.portrayal.Oriented2D {
	...
	protected final int viewRange;
	protected final int IDENTIFY_TIME;
	
	/* ********************* */
	
	private int identifyClock;

	private MutableInt2D loc;
	...
	public ExplorerAgent(MutableInt2D loc, double step, int viewRange, int identifyTime) {
		this.loc = loc;
		this.STEP = step;
		...
		if (identifyClock == 0) {
			...
			if (target != null) {
				if (loc.distance(target) == 0) {
					...
					SimObject obj = env.identifyObject(new Int2D(loc));
			...
			// Agent movement
			MutableDouble2D step = new MutableDouble2D(target.x - loc.x, target.y - loc.y);
			limit(step);
	...
	private void limit(MutableDouble2D step) {
		if(Math.sqrt((step.x*step.x)+(step.y*step.y)) > STEP){
    		step.normalize();
    		step.x *= STEP;
    		step.y *= STEP;
    	}
